...
// 3장 분할정복알고리즘 (3.1,.3.2, 3.3만 출제)
	...
	// 연습문제15번 분할정복 과정 퀵소트  6 7 3 2 9 12 17 19 13 15 14 ,피봇은 14(피봇 선정은 문제에서 요구하는대로)(+ 갤러리 참고, ppt23)
	// 14를 제일 왼쪽으로 옮기고 본래 가장 좌측에 있던 요소를 원래 14자리로vl 서로 교환
	// partition()
	// while i<=j
	...
	// 그래프 ppt
		...
		// 그래프의 순회, 그래프 탐색
			// (깊이우선 탐색 & 너비우선 탐색)-->가중치를 보지 않는 탐색방법
		...
		//setcover에서 괄호 안에 내용 넣기 정도 는 해야한다.???
			//허프만 코드에서는 예제 60번과 61번 62번 느낌을 기억하고 준비하자!(예제랑 개념만을 인지하고 알고리즘 암기는 하지 말라)
				// 별개로 이러한 허프만 코드에서 가장 작은 노드를 골라 트리를 형성할때는 "최소힙을 사용해서 가장 작은 것만을 고른다."
